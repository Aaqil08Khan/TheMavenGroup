    ...
    <div style={{ background:'var(--navy)', minHeight:'100vh', paddingTop:'100px' }}>
      ...
      <style>{`
        ...
        @media(max-width:900px){
          ...
          .ec-hero-right{display:none!important}
          .ec-stats-grid{grid-template-columns:repeat(2,1fr)!important}
          .ec-unbox-grid{grid-template-columns:1fr!important}
          .ec-cats-grid{grid-template-columns:repeat(2,1fr)!important}
          .ec-plats-grid{grid-template-columns:repeat(2,1fr)!important}
          .ec-steps{flex-wrap:wrap!important;gap:20px!important}
          .ec-section{padding:60px 24px!important}
        }
        @media(max-width:600px){
          .ec-cats-grid{grid-template-columns:1fr!important}
          .ec-plats-grid{grid-template-columns:1fr!important}
          .ec-stats-grid{grid-template-columns:repeat(2,1fr)!important}
        }
      `}</style>

      {/* HERO */}
      <section ref={heroRef} className="ec-section" style={{ padding:'80px 32px 60px', position:'relative', background:'radial-gradient(ellipse at 30% 50%,#1A0D2E 0%,#0B1C2D 70%)', overflow:'hidden' }}>
        {[...Array(20)].map((_,i) => (
          <div key={i} style={{ position:'absolute', left:`${Math.random()*100}%`, top:`${Math.random()*100}%`, width:`${Math.random()*6+2}px`, height:`${Math.random()*6+2}px`, borderRadius:Math.random()>0.5?'50%':'2px', background:['#FB7185','#A78BFA','#22D3EE','#F59E0B','#34D399'][Math.floor(Math.random()*5)], opacity:Math.random()*0.3+0.05, animation:`floatDot ${Math.random()*10+8}s ${Math.random()*5}s ease-in-out infinite alternate`, pointerEvents:'none' }}/>
        ))}
        <div style={{ position:'absolute', top:'-50px', right:'-50px', width:'400px', height:'400px', borderRadius:'50%', background:'radial-gradient(circle,rgba(251,113,133,0.08) 0%,transparent 70%)', pointerEvents:'none' }}/>
        ...
        <div style={{ maxWidth:'1280px', margin:'0 auto', position:'relative', zIndex:2 }}>
          ...
          <div className="ec-hero-grid" style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'60px', alignItems:'center' }}>
            <div>
              <div style={{ display:'inline-flex', alignItems:'center', gap:'8px', padding:'6px 16px', borderRadius:'100px', background:'rgba(251,113,133,0.1)', border:'1px solid rgba(251,113,133,0.25)', marginBottom:'24px', opacity:heroInView?1:0, transition:'all 0.5s ease 0.1s' }}>
              ...
              <p style={{ color:'#94A3B8', fontSize:'1rem', lineHeight:'1.8', maxWidth:'480px', marginBottom:'32px', opacity:heroInView?1:0, transform:heroInView?'translateY(0)':'translateY(20px)', transition:'all 0.6s ease 0.35s' }}>
                Building a cross-border retail powerhouse across India and Gulf markets — curating premium products, crafting unforgettable unboxing experiences, and scaling through <span style={{ color:'#FB7185' }}>multi-platform distribution</span>.
              </p>
              <div style={{ display:'flex', gap:'12px', flexWrap:'wrap', opacity:heroInView?1:0, transition:'all 0.6s ease 0.5s' }}>
                {['Amazon','Flipkart','D2C','Gulf Markets','Premium'].map(tag => (
                  <span key={tag} style={{ padding:'6px 14px', borderRadius:'100px', background:'rgba(251,113,133,0.08)', border:'1px solid rgba(251,113,133,0.2)', color:'#FB7185', fontSize:'12px', fontWeight:'500' }}>{tag}</span>
                ))}
              </div>
            </div>

            <div className="ec-hero-right" style={{ opacity:heroInView?1:0, transform:heroInView?'translateX(0)':'translateX(30px)', transition:'all 0.8s ease 0.3s' }}>
              <div style={{ borderRadius:'20px', background:'rgba(255,255,255,0.02)', border:'1px solid rgba(251,113,133,0.12)', padding:'28px', position:'relative', overflow:'hidden' }}>
                <div style={{ position:'absolute', inset:0, background:'radial-gradient(ellipse at 80% 20%,rgba(251,113,133,0.07) 0%,transparent 60%),radial-gradient(ellipse at 20% 80%,rgba(167,139,250,0.06) 0%,transparent 60%)' }}/>
                <div style={{ position:'relative', zIndex:1 }}>
                  ...
                  <div style={{ display:'flex', flexWrap:'wrap', gap:'8px', marginBottom:'20px' }}>
                    {categories.slice(0,4).map((cat,i) => (
                      <div key={i} style={{ padding:'6px 12px', borderRadius:'100px', background:`${cat.color}12`, border:`1px solid ${cat.color}25`, display:'flex', alignItems:'center', gap:'6px' }}>
                  ...
                  <div style={{ padding:'14px', borderRadius:'10px', background:'rgba(255,255,255,0.02)', border:'1px solid rgba(255,255,255,0.05)' }}>
                    <div style={{ color:'#475569', fontSize:'10px', letterSpacing:'1px', marginBottom:'10px' }}>ACTIVE MARKETPLACES</div>
                    <div style={{ display:'flex', gap:'10px', flexWrap:'wrap' }}>
                      {platforms.slice(0,5).map((p,i) => (
                        <div key={i} style={{ width:'32px', height:'32px', borderRadius:'8px', background:`${p.color}15`, border:`1px solid ${p.color}30`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:'16px' }}>{p.icon}</div>
                      ))}
                    </div>
                  </div>
                </div>
      ...
      {/* STATS */}
      <section className="ec-section" style={{ padding:'50px 32px', borderTop:'1px solid rgba(255,255,255,0.04)', borderBottom:'1px solid rgba(255,255,255,0.04)' }}>
      ...
      {/* UNBOXING WONDERS */}
      <section ref={unboxRef} className="ec-section" style={{ padding:'100px 32px', background:'radial-gradient(ellipse at 50% 50%,#1A0D2E 0%,#0B1C2D 70%)', position:'relative', overflow:'hidden' }}>
        <div style={{ position:'absolute', top:'-100px', left:'-100px', width:'400px', height:'400px', borderRadius:'50%', border:'1px solid rgba(251,113,133,0.06)', pointerEvents:'none' }}/>
        <div style={{ maxWidth:'1280px', margin:'0 auto', position:'relative', zIndex:2 }}>
          <div style={{ textAlign:'center', marginBottom:'20px' }}>
            <div style={{ display:'inline-flex', alignItems:'center', gap:'12px', opacity:unboxInView?1:0, transition:'all 0.5s ease' }}>
              <div style={{ width:'32px', height:'2px', background:'linear-gradient(90deg,#FB7185,#A78BFA)' }}/>
              <span style={{ color:'#FB7185', fontSize:'12px', fontWeight:'600', letterSpacing:'3px', textTransform:'uppercase' }}>Brand Identity</span>
          ...
          <div style={{ textAlign:'center', marginBottom:'60px' }}>
            ...
            <p style={{ color:'#64748B', fontSize:'1rem', maxWidth:'500px', margin:'0 auto', lineHeight:'1.8', opacity:unboxInView?1:0, transition:'all 0.6s ease 0.25s' }}>
              Every product we sell is designed to create a moment — from the first click to the last layer of packaging. We don't just sell products. We deliver experiences.
            </p>
          </div>
          <div className="ec-unbox-grid" style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'20px' }}>
            {[
              { icon:'🎁', title:'Premium Packaging', desc:'Every order ships in branded boxes designed for that perfect unboxing moment worth sharing.', color:'#FB7185', bg:'rgba(251,113,133,0.06)' },
              { icon:'💌', title:'Personal Touch',    desc:'Handwritten thank-you cards, surprise freebies, and loyalty rewards that make customers feel valued.', color:'#A78BFA', bg:'rgba(167,139,250,0.06)' },
              { icon:'📸', title:'Share-Worthy',      desc:'Products and packaging designed to be photographed, shared, and talked about on social media.', color:'#22D3EE', bg:'rgba(34,211,238,0.06)' },
            ].map((card,i) => (
      ...
      {/* CATEGORIES */}
      <section ref={categoriesRef} className="ec-section" style={{ padding:'100px 32px' }}>
        <div style={{ maxWidth:'1280px', margin:'0 auto' }}>
          <div style={{ display:'flex', alignItems:'center', gap:'12px', marginBottom:'16px', opacity:categoriesInView?1:0, transition:'all 0.5s ease' }}>
            <div style={{ width:'32px', height:'2px', background:'linear-gradient(90deg,#FB7185,#A78BFA)' }}/>
      ...
      {/* PLATFORMS */}
      <section ref={platformsRef} className="ec-section" style={{ padding:'80px 32px', background:'radial-gradient(ellipse at 50% 0%,#1A0D2E 0%,#0B1C2D 70%)', borderTop:'1px solid rgba(255,255,255,0.04)' }}>
        <div style={{ maxWidth:'1280px', margin:'0 auto' }}>
          <div style={{ display:'flex', alignItems:'center', gap:'12px', marginBottom:'16px', opacity:platformsInView?1:0, transition:'all 0.5s ease' }}>
      ...
      {/* OPERATIONS FLOW */}
      <section ref={stepsRef} className="ec-section" style={{ padding:'100px 32px' }}>
        <div style={{ maxWidth:'1280px', margin:'0 auto' }}>
          <div style={{ display:'flex', alignItems:'center', gap:'12px', marginBottom:'16px', opacity:stepsInView?1:0, transition:'all 0.5s ease' }}>
            <div style={{ width:'32px', height:'2px', background:'linear-gradient(90deg,#FB7185,#A78BFA)' }}/>
