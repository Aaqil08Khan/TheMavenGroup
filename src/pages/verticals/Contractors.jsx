...
const services = [
  { icon:'⚡', title:'Electrical Infrastructure', desc:'High-voltage installations, grid connectivity, and industrial electrical systems for large-scale Saudi projects.', color:'#F59E0B', tags:['HV Systems','Grid','Industrial'] },
  { icon:'🏗️', title:'Civil Construction', desc:'Foundation works, structural builds, and civil engineering aligned with KSA Vision 2030 mega-projects.', color:'#3B82F6', tags:['Foundation','Structural','Civil Eng'] },
  { icon:'🔧', title:'Mechanical Works', desc:'Piping, HVAC, and mechanical systems installation for industrial and commercial facilities.', color:'#22D3EE', tags:['Piping','HVAC','Mechanical'] },
  { icon:'🛡️', title:'HSE Compliance', desc:'Full Health, Safety & Environment compliance management across all active sites and contracts.', color:'#A78BFA', tags:['Safety','HSE','Compliance'] },
  { icon:'📋', title:'Contract Management', desc:'End-to-end project contracting from bid to handover — EPC, lump sum, and unit-rate models.', color:'#34D399', tags:['EPC','Lump Sum','Unit Rate'] },
  { icon:'🌐', title:'Cross-Border Sourcing', desc:'Material and manpower sourcing from India to Saudi Arabia — cost-optimized and compliant.', color:'#FB7185', tags:['India-KSA','Manpower','Sourcing'] },
]
    ...
    <div style={{ background:'var(--navy)', minHeight:'100vh', paddingTop:'100px' }}>
      ...
      <style>{`
        ...
        @media(max-width:900px){
          .con-hero-grid{grid-template-columns:1fr!important}
          .con-hero-right{display:none!important}
          .con-stats-grid{grid-template-columns:repeat(2,1fr)!important}
          .con-services-grid{grid-template-columns:repeat(2,1fr)!important}
          .con-compliance-grid{grid-template-columns:repeat(2,1fr)!important}
          .con-contract-grid{grid-template-columns:1fr!important}
          .con-section{padding:60px 24px!important}
        }
        @media(max-width:600px){
          .con-services-grid{grid-template-columns:1fr!important}
          .con-compliance-grid{grid-template-columns:1fr!important}
          .con-stats-grid{grid-template-columns:repeat(2,1fr)!important}
          .con-hero-tags{gap:8px!important}
        }
      `}</style>

      {/* HERO */}
      <section ref={heroRef} className="con-section" style={{ padding:'80px 32px 60px', position:'relative', background:'radial-gradient(ellipse at 30% 50%,#0D1F10 0%,#0B1C2D 70%)', overflow:'hidden' }}>
        <div style={{ position:'absolute', inset:0, backgroundImage:`linear-gradient(rgba(245,158,11,0.04) 1px,transparent 1px),linear-gradient(90deg,rgba(245,158,11,0.04) 1px,transparent 1px),linear-gradient(rgba(245,158,11,0.02) 1px,transparent 1px),linear-gradient(90deg,rgba(245,158,11,0.02) 1px,transparent 1px)`, backgroundSize:'100px 100px,100px 100px,20px 20px,20px 20px', animation:'blueprintScroll 20s linear infinite' }}/>
        <div style={{ position:'absolute', top:'-100px', right:'-100px', width:'500px', height:'500px', borderRadius:'50%', background:'radial-gradient(circle,rgba(245,158,11,0.07) 0%,transparent 70%)', pointerEvents:'none' }}/>
        ...
        <div style={{ maxWidth:'1280px', margin:'0 auto', position:'relative', zIndex:2 }}>
          ...
          <div className="con-hero-grid" style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'60px', alignItems:'center' }}>
            <div>
              <div style={{ display:'inline-flex', alignItems:'center', gap:'8px', padding:'6px 16px', borderRadius:'100px', background:'rgba(245,158,11,0.1)', border:'1px solid rgba(245,158,11,0.25)', marginBottom:'24px', opacity:heroInView?1:0, transition:'all 0.5s ease 0.1s' }}>
              ...
              <p style={{ color:'#94A3B8', fontSize:'1rem', lineHeight:'1.8', maxWidth:'480px', marginBottom:'32px', opacity:heroInView?1:0, transform:heroInView?'translateY(0)':'translateY(20px)', transition:'all 0.6s ease 0.35s' }}>
                Our industrial and infrastructure arm operating in the Kingdom of Saudi Arabia — delivering electrical, civil, and mechanical contracting services aligned with <span style={{ color:'#F59E0B' }}>Vision 2030</span> development corridors.
              </p>
              <div className="con-hero-tags" style={{ display:'flex', gap:'12px', flexWrap:'wrap', opacity:heroInView?1:0, transition:'all 0.6s ease 0.5s' }}>
                {['Saudi Arabia','Vision 2030','EPC','Infrastructure','HSE'].map(tag => (
                  <span key={tag} style={{ padding:'6px 14px', borderRadius:'100px', background:'rgba(245,158,11,0.08)', border:'1px solid rgba(245,158,11,0.2)', color:'#FCD34D', fontSize:'12px', fontWeight:'500' }}>{tag}</span>
                ))}
              </div>
            </div>

            <div className="con-hero-right" style={{ opacity:heroInView?1:0, transform:heroInView?'translateX(0)':'translateX(30px)', transition:'all 0.8s ease 0.3s' }}>
              <div style={{ borderRadius:'20px', background:'rgba(255,255,255,0.02)', border:'1px solid rgba(245,158,11,0.12)', padding:'32px', position:'relative', overflow:'hidden' }}>
                ...
                <div style={{ position:'absolute', inset:0, background:'radial-gradient(ellipse at 80% 20%,rgba(245,158,11,0.07) 0%,transparent 60%)' }}/>
                <div style={{ position:'relative', zIndex:1 }}>
                  ...
                  <div style={{ display:'flex', gap:'12px', alignItems:'flex-end', height:'80px', marginBottom:'16px' }}>
                    {[{label:'Electrical',h:90,color:'#F59E0B'},{label:'Civil',h:70,color:'#3B82F6'},{label:'Mechanical',h:60,color:'#22D3EE'},{label:'HSE',h:100,color:'#34D399'},{label:'Contracts',h:75,color:'#A78BFA'}].map((bar,i) => (
                      <div key={i} style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', gap:'6px' }}>
                        <div style={{ width:'100%', height:`${bar.h}%`, borderRadius:'4px 4px 0 0', background:`linear-gradient(180deg,${bar.color},${bar.color}55)`, boxShadow:`0 0 8px ${bar.color}44` }}/>
      ...
      {/* STATS */}
      <section className="con-section" style={{ padding:'50px 32px', borderTop:'1px solid rgba(255,255,255,0.04)', borderBottom:'1px solid rgba(255,255,255,0.04)' }}>
      ...
      {/* SERVICES */}
      <section ref={servicesRef} className="con-section" style={{ padding:'100px 32px' }}>
        <div style={{ maxWidth:'1280px', margin:'0 auto' }}>
          <div style={{ display:'flex', alignItems:'center', gap:'12px', marginBottom:'16px', opacity:servicesInView?1:0, transition:'all 0.5s ease' }}>
              ...
              <div key={i} onMouseEnter={() => setHoveredService(i)} onMouseLeave={() => setHoveredService(null)} style={{ padding:'28px', borderRadius:'16px', background:hoveredService===i?'rgba(255,255,255,0.05)':'rgba(255,255,255,0.02)', border:hoveredService===i?`1px solid ${service.color}44`:'1px solid rgba(255,255,255,0.05)', cursor:'default', transition:'all 0.3s ease', transform:hoveredService===i?'translateY(-6px)':servicesInView?'translateY(0)':'translateY(24px)', opacity:servicesInView?1:0, boxShadow:hoveredService===i?`0 12px 40px rgba(0,0,0,0.3),0 0 0 1px ${service.color}11`:'none', transitionDelay:servicesInView?`${i*0.08}s`:'0s', position:'relative', overflow:'hidden' }}>
                ...
                <div style={{ display:'flex', flexWrap:'wrap', gap:'6px' }}>
                  {service.tags.map(tag => (
                    <span key={tag} style={{ padding:'3px 10px', borderRadius:'100px', background:`${service.color}12`, border:`1px solid ${service.color}25`, color:service.color, fontSize:'10px', fontWeight:'500' }}>{tag}</span>
                  ))}
                </div>
              </div>
            ))}
      ...
      {/* COMPLIANCE */}
      <section ref={complianceRef} className="con-section" style={{ padding:'80px 32px', background:'radial-gradient(ellipse at 50% 0%,#0D1A10 0%,#0B1C2D 70%)', borderTop:'1px solid rgba(255,255,255,0.04)' }}>
        <div style={{ maxWidth:'1280px', margin:'0 auto' }}>
          <div style={{ display:'flex', alignItems:'center', gap:'12px', marginBottom:'16px', opacity:complianceInView?1:0, transition:'all 0.5s ease' }}>
      ...
      {/* CONTRACT MODELS */}
      <section ref={contractRef} className="con-section" style={{ padding:'100px 32px' }}>
        <div style={{ maxWidth:'1280px', margin:'0 auto' }}>
          <div style={{ display:'flex', alignItems:'center', gap:'12px', marginBottom:'16px', opacity:contractInView?1:0, transition:'all 0.5s ease' }}>
            <div style={{ width:'32px', height:'2px', background:'linear-gradient(90deg,#F59E0B,#FCD34D)' }}/>
            <span style={{ color:'#F59E0B', fontSize:'12px', fontWeight:'600', letterSpacing:'3px', textTransform:'uppercase' }}>How We Work</span>
          </div>
          ...
          <div className="con-contract-grid" style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'60px', alignItems:'center' }}>
            ...
            {/* Right — India to KSA flow */}
            <div style={{ opacity:contractInView?1:0, transform:contractInView?'translateX(0)':'translateX(30px)', transition:'all 0.8s ease 0.3s' }}>
              <div style={{ borderRadius:'20px', background:'rgba(255,255,255,0.02)', border:'1px solid rgba(245,158,11,0.1)', padding:'36px', position:'relative', overflow:'hidden' }}>
                <div style={{ position:'absolute', inset:0, backgroundImage:`linear-gradient(rgba(245,158,11,0.02) 1px,transparent 1px),linear-gradient(90deg,rgba(245,158,11,0.02) 1px,transparent 1px)`, backgroundSize:'25px 25px' }}/>
                <div style={{ position:'relative', zIndex:1 }}>
                  <div style={{ color:'#64748B', fontSize:'11px', letterSpacing:'2px', marginBottom:'24px', textTransform:'uppercase' }}>Cross-Border Operations</div>
                  <div style={{ display:'flex', alignItems:'center', gap:'16px', marginBottom:'32px' }}>
                    ...
                    <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:'4px' }}>
                      <div style={{ width:'40px', height:'2px', background:'linear-gradient(90deg,#3B82F6,#F59E0B)', position:'relative' }}>
                        <div style={{ position:'absolute', right:'-4px', top:'-3px', color:'#F59E0B', fontSize:'8px' }}>▶</div>
                      </div>
                  ...
                  {[{icon:'👷',label:'Skilled Manpower',color:'#3B82F6'},{icon:'📦',label:'Materials & Equipment',color:'#22D3EE'},{icon:'📋',label:'Project Management',color:'#F59E0B'},{icon:'🔩',label:'Technical Expertise',color:'#34D399'}].map((item,i) => (
                    <div key={i} style={{ display:'flex', alignItems:'center', gap:'12px', padding:'10px 14px', borderRadius:'8px', background:'rgba(255,255,255,0.02)', border:'1px solid rgba(255,255,255,0.04)', marginBottom:'8px' }}>
                      <span style={{ fontSize:'16px' }}>{item.icon}</span>
                      <span style={{ color:'#94A3B8', fontSize:'13px' }}>{item.label}</span>
                      <div style={{ marginLeft:'auto', width:'6px', height:'6px', borderRadius:'50%', background:item.color, boxShadow:`0 0 6px ${item.color}` }}/>
                    </div>
                  ))}
                </div>
              </div>
