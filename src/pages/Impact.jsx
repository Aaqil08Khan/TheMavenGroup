...
const initiatives = [
  { icon: '📖', title: 'Education Access', desc: 'Funding infrastructure, scholarships, and learning materials for underprivileged students in India and the Gulf.', color: '#3B82F6', points: ['School infrastructure funding', 'Annual scholarship programs', 'Book & stationery drives'] },
  { icon: '🕌', title: 'Community Institutions', desc: 'Supporting mosques, madrasas, and community centers that serve as the backbone of Islamic family life.', color: '#34D399', points: ['Madrasa infrastructure support', 'Mosque renovation funding', 'Community hall development'] },
  { icon: '🤲', title: 'Zakat & Sadaqah', desc: 'Structured religious giving programs ensuring funds reach verified beneficiaries with full transparency.', color: '#F59E0B', points: ['Verified beneficiary network', 'Transparent disbursement', 'Annual impact reporting'] },
  { icon: '👩‍⚕️', title: 'Health & Wellness', desc: 'Medical camps, health awareness programs, and emergency medical support for low-income communities.', color: '#FB7185', points: ['Free medical camps', 'Health awareness drives', 'Emergency medical aid'] },
  { icon: '💼', title: 'Livelihood Support', desc: 'Skill development and micro-enterprise support helping families achieve financial independence.', color: '#A78BFA', points: ['Vocational training', 'Micro-enterprise grants', 'Women empowerment programs'] },
  { icon: '🌱', title: 'Sustainable Giving', desc: 'Long-term community investment models designed to create self-sustaining impact ecosystems.', color: '#22D3EE', points: ['Waqf-based funding models', 'Community ownership', 'Long-term partnerships'] },
]

const stories = [
  { quote: 'The scholarship changed everything for our family. My daughter now studies engineering.', name: 'Parent, Tamil Nadu', icon: '👨‍👩‍👧', color: '#3B82F6' },
  { quote: 'The madrasa renovation meant 200 more children could learn their deen with dignity.', name: 'Imam, Kerala', icon: '🕌', color: '#34D399' },
  { quote: 'Small support, big change. The medical camp reached our entire village in one day.', name: 'Community Leader, Rajasthan', icon: '🏥', color: '#A78BFA' },
]
...
export default function Impact() {
  ...
  useEffect(() => {
    const interval = setInterval(() => setActiveStory(prev => (prev + 1) % stories.length), 3500)
    ...
    <div style={{ background: 'var(--navy)', minHeight: '100vh', paddingTop: '100px' }}>
      ...
      <style>{`
        ...
        @media(max-width:900px){
          ...
          .imp-hero-right{display:none!important}
          .imp-stats-grid{grid-template-columns:repeat(2,1fr)!important}
          .imp-init-grid{grid-template-columns:repeat(2,1fr)!important}
          .imp-stories-grid{grid-template-columns:1fr!important}
          .imp-section{padding:60px 24px!important}
          .imp-cta{padding:40px 24px!important;grid-template-columns:1fr!important}
          .imp-cta-btns{justify-content:center!important}
        }
      ...
      {/* HERO */}
      <section ref={heroRef} className="imp-section" style={{ padding:'80px 32px 60px', position:'relative', background:'radial-gradient(ellipse at 40% 50%,#0A1F15 0%,#0B1C2D 70%)', overflow:'hidden' }}>
        <div style={{ position:'absolute', top:'-80px', right:'10%', width:'400px', height:'400px', borderRadius:'50%', background:'radial-gradient(circle,rgba(52,211,153,0.07) 0%,transparent 70%)', pointerEvents:'none' }}/>
        <div style={{ position:'absolute', bottom:'-60px', left:'5%', width:'300px', height:'300px', borderRadius:'50%', background:'radial-gradient(circle,rgba(59,130,246,0.06) 0%,transparent 70%)', pointerEvents:'none' }}/>
        <div style={{ position:'absolute', inset:0, backgroundImage:'radial-gradient(rgba(52,211,153,0.08) 1px,transparent 1px)', backgroundSize:'40px 40px', pointerEvents:'none' }}/>
        ...
        <div style={{ maxWidth:'1280px', margin:'0 auto', position:'relative', zIndex:2 }}>
          ...
          <div className="imp-hero-grid" style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'60px', alignItems:'center' }}>
            <div>
              <div style={{ display:'inline-flex', alignItems:'center', gap:'8px', padding:'6px 16px', borderRadius:'100px', background:'rgba(52,211,153,0.1)', border:'1px solid rgba(52,211,153,0.25)', marginBottom:'24px', opacity:heroInView?1:0, transition:'all 0.5s ease 0.1s' }}>
              ...
              <p style={{ color:'#64748B', fontSize:'0.95rem', lineHeight:'1.8', maxWidth:'480px', marginBottom:'32px', opacity:heroInView?1:0, transition:'all 0.6s ease 0.45s' }}>
                From education funding to community health programs, from madrasa support to livelihood development — we invest in people as much as we invest in markets.
              </p>
              <div style={{ display:'flex', gap:'12px', flexWrap:'wrap', opacity:heroInView?1:0, transition:'all 0.6s ease 0.55s' }}>
                {['Education','Health','Community','Zakat','Waqf'].map(tag => (
                  <span key={tag} style={{ padding:'6px 14px', borderRadius:'100px', background:'rgba(52,211,153,0.08)', border:'1px solid rgba(52,211,153,0.2)', color:'#34D399', fontSize:'12px', fontWeight:'500' }}>{tag}</span>
                ))}
              </div>
            </div>

            <div className="imp-hero-right" style={{ opacity:heroInView?1:0, transform:heroInView?'translateX(0)':'translateX(30px)', transition:'all 0.8s ease 0.3s' }}>
              <div style={{ borderRadius:'24px', background:'rgba(255,255,255,0.02)', border:'1px solid rgba(52,211,153,0.1)', padding:'32px', position:'relative', overflow:'hidden' }}>
                ...
                <div style={{ position:'relative', zIndex:1 }}>
                  ...
                  {[{ icon:'🎓', label:'Education Programs', color:'#3B82F6' }, { icon:'🕌', label:'Community Institutions', color:'#34D399' }, { icon:'💊', label:'Health Initiatives', color:'#FB7185' }].map((item, i) => (
                    <div key={i} style={{ display:'flex', alignItems:'center', gap:'12px', padding:'10px 14px', borderRadius:'8px', background:'rgba(255,255,255,0.02)', border:'1px solid rgba(255,255,255,0.04)', marginBottom:'8px' }}>
                      ...
                      <div style={{ width:'6px', height:'6px', borderRadius:'50%', background:item.color, boxShadow:`0 0 6px ${item.color}`, animation:'pulse 2s infinite' }}/>
                    </div>
                  ))}
                </div>
              </div>
      ...
      {/* STATS */}
      <section ref={statsRef} className="imp-section" style={{ padding:'70px 32px', borderTop:'1px solid rgba(255,255,255,0.04)', borderBottom:'1px solid rgba(255,255,255,0.04)' }}>
        <div style={{ maxWidth:'1280px', margin:'0 auto' }}>
          <div className="imp-stats-grid" style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:'24px' }}>
      ...
      {/* INITIATIVES */}
      <section ref={initiativesRef} className="imp-section" style={{ padding:'100px 32px' }}>
        <div style={{ maxWidth:'1280px', margin:'0 auto' }}>
          <div style={{ display:'flex', alignItems:'center', gap:'12px', marginBottom:'16px', opacity:initiativesInView?1:0, transition:'all 0.5s ease' }}>
      ...
      {/* STORIES */}
      <section ref={storiesRef} className="imp-section" style={{ padding:'80px 32px', background:'radial-gradient(ellipse at 50% 0%,#0A1F15 0%,#0B1C2D 70%)', borderTop:'1px solid rgba(255,255,255,0.04)' }}>
        <div style={{ maxWidth:'1280px', margin:'0 auto' }}>
          <div style={{ textAlign:'center', marginBottom:'48px' }}>
            <div style={{ display:'inline-flex', alignItems:'center', gap:'12px', marginBottom:'16px', opacity:storiesInView?1:0, transition:'all 0.5s ease' }}>
              <div style={{ width:'32px', height:'2px', background:'linear-gradient(90deg,#34D399,#22D3EE)' }}/>
          ...
          <div style={{ display:'flex', justifyContent:'center', gap:'8px', marginTop:'32px' }}>
            {stories.map((story, i) => (
              <div key={i} onClick={() => setActiveStory(i)} style={{ width:activeStory===i?'24px':'8px', height:'8px', borderRadius:'100px', background:activeStory===i?story.color:'rgba(255,255,255,0.1)', cursor:'pointer', transition:'all 0.3s ease' }}/>
            ))}
          </div>
        </div>
      </section>
      ...
      {/* CTA */}
      <section ref={ctaRef} className="imp-section" style={{ padding:'100px 32px' }}>
        <div style={{ maxWidth:'1280px', margin:'0 auto' }}>
          <div style={{ borderRadius:'24px', background:'linear-gradient(135deg,rgba(52,211,153,0.08) 0%,rgba(34,211,238,0.05) 100%)', border:'1px solid rgba(52,211,153,0.15)', padding:'60px', textAlign:'center', position:'relative', overflow:'hidden', opacity:ctaInView?1:0, transform:ctaInView?'translateY(0)':'translateY(24px)', transition:'all 0.7s ease' }}>
