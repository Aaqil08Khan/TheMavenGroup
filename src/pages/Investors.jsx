...
function Counter({ to, suffix = '', duration = 1600, inView }) {
...
const PILLARS = [
  { icon:'◈', color:'#3B82F6', title:'Risk Preservation First', desc:'We never risk what we cannot afford to lose. Every allocation starts with a downside scenario before we model upside. Preservation is not a constraint — it is the strategy.' },
  { icon:'⬡', color:'#22D3EE', title:'Conviction Over Diversification', desc:'We make fewer bets and make them well. Concentrated positions in high-conviction opportunities outperform scattered exposure over a long enough horizon.' },
  { icon:'◉', color:'#A78BFA', title:'Cross-Border Compounding', desc:'Returns in one market fund expansion into the next. India funds Gulf. Gulf funds scale. Scale funds the next vertical. This is how compounding works across borders.' },
  { icon:'⬢', color:'#F59E0B', title:'Ethical Non-Negotiables', desc:'No leverage beyond mandate. No speculation disguised as investment. No conflicts of interest without full disclosure. These are not policies — they are how we sleep at night.' },
]

const ROADMAP = [
  { year:'2024', title:'Foundation Year', color:'#3B82F6', items:['Maven Business Services established','Initial Indian equity portfolio deployed','Group holding structure formalized','R&D begun on AI product suite'], status:'complete' },
  { year:'2025', title:'Build & Expand', color:'#22D3EE', items:['Maven AI Tech vertical launched','OmniX beta release to select clients','Saudi Arabia market entry initiated','HNI advisory services opened'], status:'active' },
  { year:'2026', title:'Gulf Offensive', color:'#A78BFA', items:['Full KSA & UAE operations active','Aura AI product public launch','Maven Contractors first Saudi project','E-Commerce Gulf market entry'], status:'upcoming' },
  { year:'2027', title:'Scale & Institutionalise', color:'#F59E0B', items:['Institutional investor onboarding','Regional HQ established in Gulf','AI products expanded to Bahrain','Group revenue diversification complete'], status:'upcoming' },
]

const GOVERNANCE = [
  { icon:'◈', color:'#3B82F6', title:'Investment Committee', desc:'All allocations above threshold reviewed by a structured investment committee before execution. No single-person decisions on material positions.' },
  { icon:'⬡', color:'#22D3EE', title:'Risk Framework', desc:'Proprietary risk scoring applied to every opportunity. Position sizing, stop-loss levels, and correlation limits enforced at the portfolio level.' },
  { icon:'◉', color:'#A78BFA', title:'Reporting Standards', desc:'Quarterly performance reports to all stakeholders. Full transparency on fees, returns, drawdowns, and forward-looking positioning.' },
  { icon:'⬢', color:'#F59E0B', title:'Compliance & Ethics', desc:'Fully compliant with SEBI regulations in India and applicable Gulf market regulations. External legal counsel retained across jurisdictions.' },
]

const PARTNER_TYPES = [
  { type:'HNI Investors', desc:'High net-worth individuals seeking access to Indian equity, international instruments, and Gulf real estate through a single disciplined partner.', min:'By discussion', color:'#3B82F6', icon:'◈' },
  { type:'Family Offices', desc:'Multi-generational wealth structures looking for a cross-border operating group that aligns strategy with business-building, not just financial returns.', min:'By discussion', color:'#22D3EE', icon:'⬡' },
  { type:'Strategic Partners', desc:"Businesses and operators in India, KSA, or UAE who want to co-develop verticals, share infrastructure, or access TMG's market networks.", min:'Operational', color:'#A78BFA', icon:'◉' },
]
  ...
  return (
    <div style={{ background:'var(--navy)', minHeight:'100vh', paddingTop:'100px', overflow:'hidden' }}>
      ...
      <style>{`
        ...
        @keyframes softPulse { 0%,100%{opacity:1;transform:scale(1)} 50%{opacity:.4;transform:scale(.78)} }
        ...
        @media(max-width:1024px){
          ...
          .inv-hero-right{display:none!important}
          .inv-stats-grid{grid-template-columns:repeat(2,1fr)!important}
          .inv-pillars-grid{grid-template-columns:1fr!important}
          .inv-roadmap-grid{grid-template-columns:1fr!important}
          .inv-gov-grid{grid-template-columns:1fr!important}
          .inv-gov-diagram{display:none!important}
          .inv-partners-grid{grid-template-columns:1fr!important}
          .inv-cta{grid-template-columns:1fr!important;text-align:center!important}
          .inv-section{padding:60px 24px!important}
          .inv-cta-btns{align-items:center!important}
        }
      ...
      {PARTICLES.map(p => (
        <div key={p.id} style={{ position:'fixed', left:`${p.x}%`, top:`${p.y}%`, width:`${p.size}px`, height:`${p.size}px`, borderRadius:'50%', background:p.id%3===0?'#3B82F6':p.id%3===1?'#22D3EE':'#A78BFA', opacity:p.opacity, animation:`floatP ${p.dur}s ${p.delay}s ease-in-out infinite alternate`, pointerEvents:'none', zIndex:0 }}/>
      ))}
      <div style={{ position:'fixed', inset:0, zIndex:0, pointerEvents:'none', background:'radial-gradient(ellipse at 75% 25%,rgba(26,86,219,.10) 0%,transparent 55%),radial-gradient(ellipse at 15% 75%,rgba(34,211,238,.06) 0%,transparent 50%)' }}/>
      <div style={{ position:'fixed', inset:0, zIndex:0, pointerEvents:'none', backgroundImage:`linear-gradient(rgba(59,130,246,.022) 1px,transparent 1px),linear-gradient(90deg,rgba(59,130,246,.022) 1px,transparent 1px)`, backgroundSize:'60px 60px' }}/>

      {/* ── HERO ── */}
      <section ref={heroRef} className="inv-section" style={{ padding:'72px 32px 80px', position:'relative', zIndex:1 }}>
        <div style={{ maxWidth:'1200px', margin:'0 auto' }}>
          <div className="inv-hero-grid" style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'80px', alignItems:'center' }}>
            <div>
              <div style={{ display:'inline-flex', alignItems:'center', gap:'8px', padding:'6px 16px', borderRadius:'100px', background:'rgba(59,130,246,.10)', border:'1px solid rgba(59,130,246,.25)', marginBottom:'32px', opacity:visible?1:0, transform:visible?'translateY(0)':'translateY(20px)', transition:'all .6s ease .1s' }}>
            ...
            <div className="inv-hero-right" style={{ opacity:visible?1:0, transform:visible?'translateX(0)':'translateX(36px)', transition:'all .9s ease .4s' }}>
              <div style={{ background:'rgba(11,28,45,.85)', border:'1px solid rgba(59,130,246,.18)', borderRadius:'24px', padding:'36px', backdropFilter:'blur(16px)', boxShadow:'0 24px 80px rgba(0,0,0,.45),0 0 0 1px rgba(59,130,246,.07)' }}>
                ...
                <p style={{ fontFamily:'DM Sans, sans-serif', fontSize:'14px', color:'#94A3B8', lineHeight:'1.8', marginBottom:'28px' }}>
                  Every move we make is backed by a business thesis — not short-term targets. We invest in markets we operate in, and we operate in markets we understand deeply.
                </p>
                <div style={{ display:'flex', flexWrap:'wrap', gap:'8px' }}>
                  {['Long horizon','Operational edge','Cross-border','Ethical mandate','Conviction-based'].map((tag, i) => (
                    <span key={i} style={{ padding:'5px 14px', borderRadius:'100px', background:'rgba(59,130,246,.1)', border:'1px solid rgba(59,130,246,.22)', fontFamily:'DM Sans, sans-serif', fontSize:'12px', color:'#3B82F6', fontWeight:'600' }}>{tag}</span>
                  ))}
                </div>
              </div>
            </div>
      ...
      {/* STATS */}
      <section ref={statsRef} className="inv-section" style={{ padding:'0 32px 80px', position:'relative', zIndex:1 }}>
        <div style={{ maxWidth:'1200px', margin:'0 auto' }}>
          <div className="inv-stats-grid" style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:'18px' }}>
            {[
              ...
              <div key={i} style={{ background:'rgba(255,255,255,.03)', border:`1px solid ${s.color}20`, borderRadius:'18px', padding:'28px 24px', position:'relative', overflow:'hidden', opacity:statsInView?1:0, transform:statsInView?'translateY(0)':'translateY(28px)', transition:`all .65s ease ${i*.1}s`, cursor:'default' }}
      ...
      {/* PILLARS */}
      <section ref={pillRef} className="inv-section" style={{ padding:'80px 32px', background:'rgba(255,255,255,.015)', borderTop:'1px solid rgba(255,255,255,.05)', borderBottom:'1px solid rgba(255,255,255,.05)', position:'relative', zIndex:1 }}>
        <div style={{ maxWidth:'1200px', margin:'0 auto' }}>
          ...
          <div className="inv-pillars-grid" style={{ display:'grid', gridTemplateColumns:'repeat(2,1fr)', gap:'20px' }}>
            {PILLARS.map((p, i) => (
              <div key={i} style={{ background:'rgba(255,255,255,.03)', border:'1px solid rgba(255,255,255,.07)', borderRadius:'20px', padding:'36px', opacity:pillInView?1:0, transform:pillInView?'translateY(0)':'translateY(28px)', transition:`all .65s ease ${i*.1}s`, position:'relative', overflow:'hidden', cursor:'default' }}
      ...
      {/* ROADMAP */}
      <section ref={roadRef} className="inv-roadmap-section inv-section" style={{ padding:'100px 32px', position:'relative', zIndex:1 }}>
        <div style={{ maxWidth:'1200px', margin:'0 auto' }}>
          <div style={{ display:'flex', alignItems:'center', gap:'12px', marginBottom:'20px' }}>
            <div style={{ width:'32px', height:'2px', background:'linear-gradient(90deg,#3B82F6,#22D3EE)' }}/>
          ...
          <div className="inv-roadmap-grid" style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'48px', alignItems:'start' }}>
            <div>
              {ROADMAP.map((r, i) => (
                <div key={r.year} onClick={() => setActiveYear(r.year)} style={{ display:'flex', gap:'20px', alignItems:'flex-start', padding:'24px 28px', borderRadius:'16px', cursor:'pointer', marginBottom:'12px', background:activeYear===r.year?r.color+'0E':'rgba(255,255,255,.02)', border:`1px solid ${activeYear===r.year?r.color+'45':'rgba(255,255,255,.06)'}`, transition:'all .25s ease', opacity:roadInView?1:0, transform:roadInView?'translateX(0)':'translateX(-24px)', transitionDelay:`${i*.1}s` }}
                  ...
                  <div style={{ flexShrink:0, marginTop:'3px' }}>
                    <div style={{ width:'12px', height:'12px', borderRadius:'50%', background:r.status==='complete'?r.color:r.status==='active'?r.color:'transparent', border:`2px solid ${r.color}`, boxShadow:r.status==='active'?`0 0 10px ${r.color}`:'none' }}/>
                  </div>
                  <div style={{ flex:1 }}>
            ...
            <div key={activeYear} style={{ animation:'fadeUp .35s ease', opacity:roadInView?1:0, transform:roadInView?'translateX(0)':'translateX(24px)', transition:'opacity .8s ease .3s,transform .8s ease .3s' }}>
              <div style={{ background:'rgba(255,255,255,.03)', border:`1px solid ${activeRoadmap.color}30`, borderRadius:'20px', padding:'40px', position:'relative', overflow:'hidden' }}>
                ...
                <div style={{ display:'flex', flexDirection:'column', gap:'14px' }}>
                  {activeRoadmap.items.map((item, i) => (
                    <div key={i} style={{ display:'flex', alignItems:'flex-start', gap:'14px', padding:'16px 18px', borderRadius:'12px', background:'rgba(255,255,255,.025)', border:'1px solid rgba(255,255,255,.05)', animation:`fadeUp .4s ease ${i*.08}s both` }}>
                      <div style={{ width:'7px', height:'7px', borderRadius:'50%', background:activeRoadmap.color, marginTop:'5px', flexShrink:0, boxShadow:`0 0 7px ${activeRoadmap.color}` }}/>
      ...
      {/* GOVERNANCE */}
      <section ref={govRef} className="inv-section" style={{ padding:'80px 32px', background:'rgba(255,255,255,.015)', borderTop:'1px solid rgba(255,255,255,.05)', borderBottom:'1px solid rgba(255,255,255,.05)', position:'relative', zIndex:1 }}>
        <div style={{ maxWidth:'1200px', margin:'0 auto' }}>
          <div style={{ display:'flex', alignItems:'center', gap:'12px', marginBottom:'20px' }}>
            <div style={{ width:'32px', height:'2px', background:'linear-gradient(90deg,#3B82F6,#22D3EE)' }}/>
            <span style={{ color:'#22D3EE', fontSize:'12px', fontWeight:'600', letterSpacing:'3px', textTransform:'uppercase', fontFamily:'DM Sans, sans-serif' }}>Governance Model</span>
          </div>
          <div className="inv-gov-grid" style={{ display:'grid', gridTemplateColumns:'0.9fr 1.1fr', gap:'80px', alignItems:'start' }}>
            <div style={{ opacity:govInView?1:0, transform:govInView?'translateX(0)':'translateX(-24px)', transition:'all .8s ease' }}>
              ...
              <div className="inv-gov-diagram" style={{ position:'relative', height:'220px', display:'flex', alignItems:'center', justifyContent:'center' }}>
                ...
                {[{ label:'Investment\nCommittee', angle:0, r:95 }, { label:'Vertical\nLeads', angle:120, r:95 }, { label:'External\nAdvisory', angle:240, r:95 }].map((item, i) => {
                  ...
                  return (
                    <div key={i} style={{ position:'absolute', left:`calc(50% + ${x}px)`, top:`calc(50% + ${y}px)`, transform:'translate(-50%,-50%)', fontFamily:'DM Sans, sans-serif', fontSize:'10px', color:'#64748B', textAlign:'center', lineHeight:'1.4', whiteSpace:'pre-line', fontWeight:'600' }}>
                      {item.label}
                    </div>
                  )
                })}
            ...
            <div style={{ opacity:govInView?1:0, transform:govInView?'translateX(0)':'translateX(24px)', transition:'all .8s ease .2s' }}>
              {GOVERNANCE.map((g, i) => (
                <div key={i} className="gov-item" style={{ display:'flex', gap:'18px', padding:'22px 24px', borderRadius:'14px', marginBottom:'12px', border:'1px solid rgba(255,255,255,.06)', background:'rgba(255,255,255,.025)', opacity:govInView?1:0, transform:govInView?'translateY(0)':'translateY(16px)', transition:`all .6s ease ${i*.1}s`, cursor:'default' }}
      ...
      {/* PARTNERS */}
      <section ref={partRef} className="inv-partner-section inv-section" style={{ padding:'100px 32px 120px', position:'relative', zIndex:1 }}>
        <div style={{ maxWidth:'1200px', margin:'0 auto' }}>
          <div style={{ display:'flex', alignItems:'center', gap:'12px', marginBottom:'20px' }}>
            <div style={{ width:'32px', height:'2px', background:'linear-gradient(90deg,#3B82F6,#22D3EE)' }}/>
